...
interface ProjectProps {
  ...
  productOwnerIDInit: string;
  scrumMasterIDInit: string;
  developersInit: string[];
...
const ProjectForm: React.FC<ProjectProps> = ({
  ...
  const submitFormHandler = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!formIsValid) {
      setInvalidFormMessage("Make sure to fill out all required fields."); // TODO remove this
  ...
  return (
    <Fragment>
      <h1 className={`${classes.cardHeading} text-primary`}>{formTitle}</h1>
